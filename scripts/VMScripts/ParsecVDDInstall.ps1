...
(New-Object System.Net.WebClient).DownloadFile("https://builds.parsec.app/vdd/parsec-vdd-0.41.0.0.exe", "C:\Users\$env:USERNAME\Downloads\parsec-vdd.exe")
    while ($NULL -eq ((Get-ChildItem Cert:\LocalMachine\TrustedPublisher) | Where-Object { $_.Subject -like '*Parsec*' })) {
        certutil -Enterprise -Addstore "TrustedPublisher" C:\ProgramData\HyperGame\ParsecPublic.cer
        Start-Sleep -s 5
    }
